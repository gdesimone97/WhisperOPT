...
def forward_encoder(model):
    ...
    for i, sample in enumerate(dataset_iter):
        ...
        inputs["input_features"] = inputs["input_features"].to(encoder.dtype)
        out = model(**inputs)
...
if __name__ == "__main__":
    ...
    assert encoder.dtype == dtype, f"Encoder dtype {encoder.dtype} does not match model dtype {dtype}"
    whisper_model = WhisperForConditionalGeneration.from_pretrained(model_name,
                                                              torch_dtype=dtype,
                                                              device_map="cuda",
                                                              )
    decoder = whisper_model.get_decoder()
    assert decoder.dtype == dtype, f"Decoder dtype {decoder.dtype} does not match model dtype {dtype}"
    decoder = Decoder(decoder, whisper_model.proj_out).to("cuda")
    processor = WhisperProcessor.from_pretrained(model_name)
    processor.tokenizer.set_prefix_tokens(language="en", task="transcribe")
    config = mtq.INT8_DEFAULT_CFG
    dummy = torch.randn(1, 128, 3000).to("cuda", dtype=encoder.dtype)
    ...
    enc_output = encoder(dummy).last_hidden_state.to("cuda", dtype=dtype)   
    dummy = {
